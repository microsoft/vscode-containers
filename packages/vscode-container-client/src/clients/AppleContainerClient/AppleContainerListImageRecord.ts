 ...
 * repository/tag live in `configuration.descriptor.name` (e.g.
 ...
 * than double-counting undownloaded platforms.
 */
...
export function normalizeAppleContainerListImageRecord(image: AppleContainerListImageRecord): ListImagesItem {
    const size = (image.variants ?? []).reduce((total, variant) => total + (variant.size ?? 0), 0);

    return {
        id: image.id,
        image: parseDockerLikeImageName(image.configuration.descriptor.name),
