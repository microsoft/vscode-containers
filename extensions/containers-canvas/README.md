...
pnpm --filter containers-canvas build     # produces dist/
pnpm --filter containers-canvas test      # pure unit tests, no daemon required
```

`dist/` is committed so the package installs without a build step. Rebuild it
...
`dist/`, which is committed and installed with no build step, so those licences
