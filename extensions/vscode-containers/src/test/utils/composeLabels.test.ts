...
import { getComposeEnvFile, getComposeFiles, getComposeProjectName, getComposeServiceName, getComposeWorkingDirectory } from '../../utils/composeLabels';

suite("(unit) composeLabels", () => {
    suite("getComposeFiles", () => {
        ...
        test("Returns undefined when the config files label is absent or labels undefined", () => {
            expect(getComposeFiles({})).to.be.undefined;
            expect(getComposeFiles(undefined)).to.be.undefined;
        });
    });
    ...
    suite("other label accessors", () => {
        ...
        test("getComposeServiceName returns the service name", () => {
            expect(getComposeServiceName(labels)).to.equal('web');
        });

        test("Accessors return undefined when their label is absent or labels undefined", () => {
            ...
            expect(getComposeServiceName({})).to.be.undefined;
            expect(getComposeProjectName(undefined)).to.be.undefined;
            expect(getComposeWorkingDirectory(undefined)).to.be.undefined;
            expect(getComposeEnvFile(undefined)).to.be.undefined;
            expect(getComposeServiceName(undefined)).to.be.undefined;
        });
    });
});
