...
suite('(unit) containers.closeComposeTaskTerminal', () => {
    test('defaults to false', () => {
        const inspected = vscode.workspace.getConfiguration(configPrefix).inspect<boolean>('closeComposeTaskTerminal');

        expect(inspected?.defaultValue).to.equal(false);
    });
});
